...
#'   at 30% tint, "#CCECF9".
#' @param colour_default Colour of the other countries without trial data,
...
create_studies_map <- function(data, country_col,
  ...
  sum_trials = sum(group_df$n_trials)
  ...
  plot = ggplot(world_map_df, aes(x = long, y = lat, group = group)) +
    ...
    geom_polygon(data = world_map_df %>%
                 ...
                 mapping = aes(x = long, y = lat, group = group, fill = n_trials),
  ...
  if(include_n == TRUE){
    plot = plot +
      annotate("text", x = 165, y = -60, label = str_c("n = ", sum_trials))
  }
