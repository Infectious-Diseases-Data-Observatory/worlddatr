...
If your dataset only has country name or doesn't include the ISO three letter code required for the `create_map` function to work, the `convert_country_to_iso()` function takes various spellings of country names and matches them with the standardised ISO code. 

Should the spelling of any country not match with a standardised ISO code, the R console will report the number of unmatched rows. If you find spellings or alternatives not covered in our data bank, please raise this as an issue in the [worlddatr github](https://github.com/Infectious-Diseases-Data-Observatory/worlddatr/issues) and we will add them and strengthen the function and data bank further.
